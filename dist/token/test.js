import splToken from '@solana/spl-token/lib/index.cjs.js';
 ...
 */
console.log('Creating a new token mint.', fromWallet.publicKey, splToken.TOKEN_PROGRAM_ID.toBase58());
const mint = await splToken.Token.createMint(connection, fromWallet, fromWallet.publicKey, null, 9, splToken.TOKEN_PROGRAM_ID);
/**
...
await mint.mintTo(fromTokenAccount.address, fromWallet.publicKey, [], 1000000000);
/**
 * Create the transaction from the sender to the receiver.
 */
console.log('Creating transaction.');
const transaction = new web3.Transaction().add(splToken.Token.createTransferInstruction(splToken.TOKEN_PROGRAM_ID, fromTokenAccount.address, toTokenAccount.address, fromWallet.publicKey, [], 1));
/**
 * Sign and broadcast the transaction.
 */
console.log('Signing transaction and broadcasting.');
const signature = await web3.sendAndConfirmTransaction(connection, transaction, [fromWallet], { commitment: 'confirmed' });
